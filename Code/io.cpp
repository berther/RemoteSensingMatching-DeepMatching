...
int every_score = 0;
float match_iamge_shift_x = 0;
float match_iamge_shift_y = 0;
int get_every_image_score(void)
{
    return every_score;
}

float get_image_shift_x(void)
{
    return match_iamge_shift_x;
}

float get_image_shift_y(void)
{
    return match_iamge_shift_y;
}

void output_correspondences( const char* out_filename, const corres_t* corres, int nb, float fx, float fy ) 
{
    every_score = 0;
    match_iamge_shift_x = 0;
    match_iamge_shift_y = 0;
  assert(0<fx && fx<=2);
  assert(0<fy && fy<=2);
  ...
  for(int i=0; i<nb; i++) 
  {
    ...
    //every_score = every_score + r->score;
    every_score = every_score + r->maxima;
    // if (f)
    //   fprintf(f,"%g %g %g %g %g %g\n",fx*r->x0,fy*r->y0,fx*r->x1,fy*r->y1,r->maxima,r->score);
    // else
    //   std_printf("%g %g %g %g %g %g\n",fx*r->x0,fy*r->y0,fx*r->x1,fy*r->y1,r->maxima,r->score);
    //std_printf("%g %g %g %g %g %g\n",fx*r->x0,fy*r->y0,fx*r->x1,fy*r->y1,r->maxima,r->score);
    match_iamge_shift_x = match_iamge_shift_x + fx*r->x1 - fx*r->x0;
    match_iamge_shift_y = match_iamge_shift_y + fy*r->y1 - fy*r->y0;
    }
    match_iamge_shift_x = match_iamge_shift_x/nb;
    match_iamge_shift_y = match_iamge_shift_y/nb;

    //std_printf("%g\n",nb);
    //std_printf("%g\n",match_iamge_shift_y);
    
    //std_printf("every_image_score:%d\n",every_score);
